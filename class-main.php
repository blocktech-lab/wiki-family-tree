...
class Main {
	public function __construct() {
		...
		load_plugin_textdomain( 'wiki-family-tree' );
	...
	public static function activate_for_blog( $blog_id ) {
		...
		if ( is_plugin_active_for_network( 'wiki-family-tree/wiki-family-tree.php' ) ) {
	...
	public static function setup_roles() {
		$default_roles = array(
			'wiki-user'   => _x( 'Wiki User', 'User role', 'wiki-family-tree' ),
			'wiki-editor' => _x( 'Wiki Editor', 'User role', 'wiki-family-tree' ),
		);
	...
	public static function get_role_capabilities( $role ) {
		...
		foreach ( array_keys( $capabilities ) as $type ) {
			$capabilities[ $type ]['wiki-family-tree'] = true;
		}
