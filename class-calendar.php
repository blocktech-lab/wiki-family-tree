				...
				foreach ( $dates as $type => $date ) {
					...
					if ( 'born' === $type ) {
						...
						// translators: %1$s is a name, %2%s is a date.
							__( '%1$s was born on %2$s', 'wiki-family-tree' ),
							$arr['text'],
						...
						if ( get_field( 'alive', $page->ID ) ) {
							if ( $date->format( 'm' ) < $now->format( 'm' ) || ( $date->format( 'm' ) === $now->format( 'm' ) && $date->format( 'j' ) < $now->format( 'j' ) ) ) {
								...
								if ( $age->y ) {
									...
									$age = sprintf( _n( 'turned %d', 'turned %d', $age->y, 'wiki-family-tree' ), $age->y );
								} else {
									$age = __( 'was born', 'wiki-family-tree' );
								}
							} elseif ( $date->format( 'm-d' ) === $now->format( 'm-d' ) ) {
								$age = $date->diff( $now );
								if ( $age->y ) {
									// translators: %d is an age in years.
									$age = sprintf( _n( 'turns %d today', 'turns %d today', $age->y, 'wiki-family-tree' ), $age->y );
								} else {
									$age = __( 'was born today', 'wiki-family-tree' );
								}
							} else {
								$age = $now->format( 'Y' ) - $date->format( 'Y' );
								// translators: %d is an age in years.
								$age = sprintf( _n( 'will turn %d', 'will turn %d', $age, 'wiki-family-tree' ), $age );
							}
							$arr['age'] = $age;
							$arr['text'] .= ' (' . $age . ')';
						} else {
							...
							$arr['text'] .= ' (' . sprintf( _n( '%d years ago', '%d years ago', $age, 'wiki-family-tree' ), $age ) . ')';
						}
					} else {
						$arr['text'] = sprintf(
						// translators: %1$s is a name, %2%s is a date.
							__( '%1$s died on %2$s', 'wiki-family-tree' ),
							$arr['text'],
							date_i18n( get_option( 'date_format' ), $date->format( 'U' ) )
						);
						$age = $now->format( 'Y' ) - $date->format( 'Y' );
	...
	public function render_family_calendar() {
		...
		foreach ( $dates as $date => $people ) {
			foreach ( $people as $person ) {
				...
				$return .= '<li>' . wp_kses_post( str_replace( array( ' (' . __( '0 years ago', 'wiki-family-tree' ) . ')', ' (' . __( 'was born', 'wiki-family-tree' ) . ')' ), ' (' . __( 'this year', 'wiki-family-tree' ) . ')', $person['text'] ) ) . '.</li>';
			}
		}
