...
class Private_Site {
	...
	public function blog_privacy_selector() {
		...
		<label for="blog-private"><?php esc_html_e( 'Private, visible only to administrators, editors, and wiki users', 'wiki-family-tree' ); ?></label>
		<?php
	}
	...
	public function parse_request() {
		...
		<html>
		<head>
		<title><?php esc_html_e( 'This site is private.', 'wiki-family-tree' ); ?></title>
		</head>
		<body>
			<?php esc_html_e( 'This site is private.', 'wiki-family-tree' ); ?>
	...
	public function rest_dispatch_request( $dispatch_result, $request, $route ) {
		...
		if ( get_option( 'blog_public' ) === -1 ) {
			return new \WP_Error( 'private_site', __( 'This site is private.', 'wiki-family-tree' ), array( 'status' => 403 ) );
		}
	...
	public function opml_head() {
		status_header( 403 );
		?>
		<error><?php esc_html_e( 'This site is private.', 'wiki-family-tree' ); ?></error>
	</head>
</opml>
		<?php
		exit;
	...
	public function bloginfo( $value, $what ) {
		if ( ( 'name' === $what || 'title' === $what ) && $this->is_private() ) {
			return __( 'This site is private.', 'wiki-family-tree' );
		}
