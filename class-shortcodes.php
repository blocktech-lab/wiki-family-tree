...
class Shortcodes {
	...
	public function short_bio_children() {
		...
		if ( 2 === count( $children ) ) {
			if ( 'Male' === get_field( 'sex' ) ) {
				return sprintf(
					...
					__( 'father of %1$s and %2$s', 'wiki-family-tree' ),
			...
			if ( 'Female' === get_field( 'sex' ) ) {
				return sprintf(
					// translators: %1$s is a first child's name, %2$s is a second child's name.
					__( 'mother of %1$s and %2$s', 'wiki-family-tree' ),
					array_shift( $children ),
					array_shift( $children )
				);
			}
			...
			return sprintf(
				// translators: %1$s is a first child's name, %2$s is a second child's name.
				__( 'parent of %1$s and %2$s', 'wiki-family-tree' ),
				array_shift( $children ),
				array_shift( $children )
			);
		}
		...
		if ( 'Male' === get_field( 'sex' ) ) {
			return sprintf(
				...
				__( 'father of %s', 'wiki-family-tree' ),
				$last_child
			);
		}
		if ( 'Female' === get_field( 'sex' ) ) {
			return sprintf(
				// translators: %s is a child
				__( 'mother of %s', 'wiki-family-tree' ),
				$last_child
			);
		}
		...
		return sprintf(
			// translators: %s is a child
			__( 'parent of %s', 'wiki-family-tree' ),
			$last_child
		);
	}
	public function short_bio_siblings() {
		...
		if ( $siblings ) {
			if ( 'Male' === get_field( 'sex' ) ) {
				$return[] = sprintf(
					...
					__( 'brother of %s', 'wiki-family-tree' ),
					implode( ', ', $siblings )
				);
			} elseif ( 'Female' === get_field( 'sex' ) ) {
				$return[] = sprintf(
					// translators: %s is a list of siblings.
					__( 'sister of %s', 'wiki-family-tree' ),
					implode( ', ', $siblings )
				);
			} else {
				$return[] = sprintf(
					// translators: %s is a list of siblings.
					__( 'sibling of %s', 'wiki-family-tree' ),
					implode( ', ', $siblings )
				);
			}
		}
		if ( $half_siblings ) {
			if ( 'Male' === get_field( 'sex' ) ) {
				$return[] = sprintf(
					// translators: %s is a list of siblings.
					__( 'half-brother of %s', 'wiki-family-tree' ),
					implode( ', ', $half_siblings )
				);
			} elseif ( 'Female' === get_field( 'sex' ) ) {
				$return[] = sprintf(
					// translators: %s is a list of siblings.
					__( 'half-sister of %s', 'wiki-family-tree' ),
					implode( ', ', $half_siblings )
				);
			} else {
				$return[] = sprintf(
					// translators: %s is a list of half-siblings.
					__( 'half-sibling of %s', 'wiki-family-tree' ),
					implode( ', ', $half_siblings )
				);
			}
		}
	...
	public function short_bio_parents() {
		...
		if ( 'Male' === get_field( 'sex' ) ) {
			return sprintf(
				...
				__( 'son of %1$s and %2$s', 'wiki-family-tree' ),
				$mother,
				$father
			);
		}
		if ( 'Female' === get_field( 'sex' ) ) {
			return sprintf(
				// translators: %1$s is a mother's name, %2$s is a father's name.
				__( 'daughter of %1$s and %2$s', 'wiki-family-tree' ),
				$mother,
				$father
			);
		}
		...
		return sprintf(
			// translators: %1$s is a mother's name, %2$s is a father's name.
			__( 'child of %1$s and %2$s', 'wiki-family-tree' ),
			$mother,
			$father
		);
	}
	...
	public function short_date_bio() {
		...
		if ( get_field( 'alive' ) ) {
			...
			$age_ca_placeholder = sprintf(
				...
				_n( 'age: ~%d', 'age: ~%d', $age->y, 'wiki-family-tree' ),
				$age->y
			);
			...
			if ( get_field( 'born_as' ) ) {
				if ( get_field( 'birth_place' ) ) {
					if ( get_field( 'exact_birth_date_unknown' ) ) {
						return sprintf(
							...
							__( 'born as %1$s in %2$s (%3$s) in %4$s', 'wiki-family-tree' ),
					...
					return sprintf(
						...
						__( 'born as %1$s on %2$s (%3$s) in %4$s', 'wiki-family-tree' ),
						'<i>' . esc_html( get_field( 'born_as' ) ) . '</i>',
						$this->get_date( $birth ),
						$age_placeholder,
						esc_html( get_field( 'birth_place' ) )
				...
				if ( get_field( 'exact_birth_date_unknown' ) ) {
					return sprintf(
						...
						__( 'born as %1$s in %2$s (%3$s)', 'wiki-family-tree' ),
						'<i>' . esc_html( get_field( 'born_as' ) ) . '</i>',
						$birth->format( 'Y' ),
						$age_ca_placeholder
					);
				...
				return sprintf(
					...
					__( 'born as %1$s on %2$s (%3$s)', 'wiki-family-tree' ),
					'<i>' . esc_html( get_field( 'born_as' ) ) . '</i>',
					$this->get_date( $birth ),
					$age_placeholder
				);
			...
			if ( get_field( 'birth_place' ) ) {
				if ( get_field( 'exact_birth_date_unknown' ) ) {
					return sprintf(
						...
						__( 'born in %1$s (%2$s) in %3$s', 'wiki-family-tree' ),
						$birth->format( 'Y' ),
						$age_ca_placeholder,
						esc_html( get_field( 'birth_place' ) )
					);
				...
				return sprintf(
					...
					__( 'born on %1$s (%2$s) in %3$s', 'wiki-family-tree' ),
					$this->get_date( $birth ),
					$age_placeholder,
					esc_html( get_field( 'birth_place' ) )
				);
			...
			if ( get_field( 'exact_birth_date_unknown' ) ) {
				return sprintf(
					...
					__( 'born in %1$s (%2$s)', 'wiki-family-tree' ),
					$birth->format( 'Y' ),
					$age_ca_placeholder
				);
			}
			return sprintf(
				...
				__( 'born on %1$s (%2$s)', 'wiki-family-tree' ),
				$this->get_date( $birth ),
				$age_placeholder
			);
		}
			...
			if ( get_field( 'birth_place' ) ) {
				if ( get_field( 'exact_birth_date_unknown' ) ) {
					$return = sprintf(
						...
						__( 'born as %1$s in %2$s in %3$s', 'wiki-family-tree' ),
						'<i>' . esc_html( get_field( 'born_as' ) ) . '</i>',
						$birth->format( 'Y' ),
						esc_html( get_field( 'birth_place' ) )
					);
					...
					$return = sprintf(
						...
						__( 'born as %1$s on %2$s in %3$s', 'wiki-family-tree' ),
						'<i>' . esc_html( get_field( 'born_as' ) ) . '</i>',
						$this->get_date( $birth ),
						esc_html( get_field( 'birth_place' ) )
					);
				...
				if ( get_field( 'exact_birth_date_unknown' ) ) {
					$return = sprintf(
						...
						__( 'born as %1$s in %2$s', 'wiki-family-tree' ),
						'<i>' . esc_html( get_field( 'born_as' ) ) . '</i>',
						$birth->format( 'Y' )
					);
				} else {
					$return = sprintf(
						...
						__( 'born as %1$s on %2$s', 'wiki-family-tree' ),
						'<i>' . esc_html( get_field( 'born_as' ) ) . '</i>',
						$this->get_date( $birth )
					);
				}
			...
			if ( get_field( 'birth_place' ) ) {
				if ( get_field( 'exact_birth_date_unknown' ) ) {
					$return = sprintf(
						...
						__( 'born in %1$s in %2$s', 'wiki-family-tree' ),
						$birth->format( 'Y' ),
						esc_html( get_field( 'birth_place' ) )
					);
				} else {
					$return = sprintf(
						...
						__( 'born on %1$s in %2$s', 'wiki-family-tree' ),
						$this->get_date( $birth ),
						esc_html( get_field( 'birth_place' ) )
					);
				}
				...
				if ( get_field( 'exact_birth_date_unknown' ) ) {
					$return = sprintf(
						...
						__( 'born in %s', 'wiki-family-tree' ),
						$birth->format( 'Y' )
					);
				} else {
					$return = sprintf(
						// translators: %s is a birth date.
						__( 'born on %s', 'wiki-family-tree' ),
						$this->get_date( $birth )
					);
				}
			}
		...
		if ( get_field( 'birth_date' ) && get_field( 'death_date' ) ) {
			...
			$aged_placeholder = sprintf(
				// translators: %s is an age in years.
				_n( 'aged: %d', 'aged: %d', $aged->y, 'wiki-family-tree' ),
				$aged->y
			);
			...
			if ( get_field( 'death_place' ) ) {
				if ( get_field( 'exact_death_date_unknown' ) ) {
					return $return . ', ' . sprintf(
						...
						__( 'died in %1$s (%2$s) in %3$s', 'wiki-family-tree' ),
						$death->format( 'Y' ),
						$aged_ca_placeholder,
				...
				return $return . ', ' . sprintf(
					...
					__( 'died on %1$s (%2$s) in %3$s', 'wiki-family-tree' ),
					$this->get_date( $death ),
					$aged_placeholder,
					esc_html( get_field( 'death_place' ) )
				);
			...
			if ( get_field( 'exact_death_date_unknown' ) ) {
				return $return . ', ' . sprintf(
					...
					__( 'died in %1$s (%2$s)', 'wiki-family-tree' ),
					$death->format( 'Y' ),
					$aged_ca_placeholder
				);
			}
			return $return . ', ' . sprintf(
				...
				__( 'died on %1$s (%2$s)', 'wiki-family-tree' ),
				$this->get_date( $death ),
				$aged_placeholder
			);
		}
	...
	public function died( $atts, $content ) {
		...
		return $return . ' (' . sprintf(
		// translators: %s is an age in years.
			_n( 'aged %d', 'aged %d', $age->y, 'wiki-family-tree' ),
			$age->y
		) . ')';
	}
}
